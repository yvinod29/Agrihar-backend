    ...
    >
      ...
      >
        <div className="p-2 flex justify-between">
          ...
          {/* Desktop Menu */}
          <div className="hidden md:flex gap-4 border border-gray-300 rounded-full py-2 px-3 shadow-md shadow-gray-300">
          ...
          <div>
            {isAuthenticated ? (
              <div className="flex gap-2">
                <Link to="/host">
                  <div className="md:block text-sm font-semibold py-2 px-3 rounded-full bg-primary text-white  hover:bg-white hover:text-black transition cursor-pointer shadow-md border-neutral-50">
                     BECOME  A HOST
                  </div>
                </Link>
                <div className="p-2 md:p-1 md:px-2 border-[1px] border-neutral-200 flex flex-row items-center gap-2 rounded-full cursor-pointer hover:shadow-md transition">
                  ...
                  <Link to="/profile">
                    <div className="hidden md:flex">{user.firstName}</div>
                  </Link>
                </div>
              </div>
            ) : (
          ...
          {/* Responsive Menu */}
          <div className="md:hidden block text-sm font-semibold py-1 px-2 rounded-full hover:bg-primary hover:text-white transition cursor-pointer shadow-md border-neutral-50">
            {isMenuOpen ? (
              <FaXmark
                onClick={toggleMenu}
