...
const Wedding = () => {
    ...
    const formatDateRange = (startDate, endDate) => {
        ...
        const endMonth = new Date(endDate).toLocaleString('en-us', { month: 'short' });
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 p-2 ">
                {weddings && weddings.weddings ? (
                    weddings.weddings.map((wedding) => {
                        ...
                        const eventStartTimes = wedding.events.map(event => new Date(event.startingTime));
                        const earliestStartTime = new Date(Math.min(...eventStartTimes));
                        const latestStartTime = new Date(Math.max(...eventStartTimes));

                        return (
                            <div key={wedding._id} className="bg-white p-4 rounded-md shadow-md flex flex-col items-center">
                            <Link to={`/wedding/${wedding._id}`} className="text-center">
                                <div className="relative">
                                    <img src={wedding.images[0].secureUrl} alt="Wedding Image" className="w-[400px] h-[350px] rounded-md" />
                                    <div className="absolute bottom-0 left-2 right-0 text-left py-2">
                                        <h2 className="text-3xl font-bold text-white">
                                            {wedding.brideFirstName} & {wedding.groomFirstName}&apos;s Wedding
                                        </h2>
                                    </div>
                                </div>
                                <h3 className="text-center">{wedding.events[0].city}</h3>
                                <h3 className="text-center">{formatDateRange(earliestStartTime, latestStartTime)}</h3>
                            </Link>
